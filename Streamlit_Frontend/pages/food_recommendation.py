...
# Add this near the top of your file, right after the st.set_page_config line:
st.set_page_config(page_title="Automatic Food Recommendation", page_icon="💪", layout="wide")

# Add this CSS for background image
st.markdown("""
    <style>
        .stApp {
            background-image: url("https://www.pexels.com/photo/sliced-bread-on-brown-wooden-chopping-board-13983276/f");
            background-size: cover;
            background-repeat: no-repeat;
            background-attachment: fixed;
        }
        
        /* Make content more readable with semi-transparent white background */
        .block-container {
...
class Display:
    ...
    def display_recommendation(self,person,recommendations):
        ...
        with st.spinner('Generating recommendations...'): 
            ...
            # Add styling for meal sections and recipes
            st.markdown("""
                <style>
                .meal-section {
                    padding: 20px;
            ...
            # Display each meal section vertically
            for meal_name, recommendation in zip(meals, recommendations):
                #st.markdown(f'<div class="meal-section">', unsafe_allow_html=True)
                st.markdown(f'<div class="meal-title">{meal_name.upper()}</div>', unsafe_allow_html=True)
                
                # Display recipes vertically within each meal section
                for recipe in recommendation:
...
display=Display()
title="<h1 style='text-align: center;'>Automatic Food Recommendation</h1>"
